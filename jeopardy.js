...
async function getCategoryIds() {
    ...
    const ids = _.shuffle(response.data.map(category => category.id));
    return ids.slice(0,numCategories);
}
...
async function getCategory(catId) {
    const clues = [];
    const obj = await axios.get(`https://rithm-jeopardy.herokuapp.com/api/category?id=${catId}`);
    let category = obj.data;
    // console.log(category);
    for (let i = 0; i < numQuestionsPerCat; i++) {
        
        clues.push({question: category.clues[i].question, answer: category.clues[i].answer, showing: null});
    }
